...
```{r}
yearly_scope2 <- plant_emissions_long %>% 
  group_by(plant_id, plant_name, address) %>% 
...
checking_unique_zip_codes_scope_2 <- zipcode_extraction %>% 
  group_by(zip_code) %>% 
  summarise(count = n()) %>% 
  filter(count < 2) %>% 
  drop_na() %>%
...
unique_scope2_zip_codes <- checking_unique_zip_codes_scope_2 %>% 
...
checking_unique_zip_codes_scope_1 <- scope1 %>% 
  group_by(zip_code) %>% 
  summarise(count = n()) %>% 
  filter(count < 2) %>% 
  ungroup() %>% 
...
unique_scope1_zip_codes <- checking_unique_zip_codes_scope_1 %>% 
...
scope_1_and_2_matches <- unique_scope1_zip_codes %>% 
...
```{r}
remaining_zip_codes <-  zipcode_extraction %>% 
  anti_join(scope_1_and_2_matches, by = "zip_code")

## Making sure zip codes match if they are in 
corrected_ghgids <- remaining_zip_codes %>% 
...
scope_1_and_2_matches_complete <- scope_1_and_2_matches %>% 
  full_join(corrected_ghgids) %>% 
  right_join(zipcode_extraction, by = "plant_id")
```

## OKAY NOW THEY ARE ALMOST FULLY JOINED. WE JUST NEED TO GET THE FINAL SCOPE 1 AMOUNTS JOINED ONTO THEM TOO AND THEN WE ARE DONE
