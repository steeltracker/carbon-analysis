...
zip_code_tidy <- yearly_scope2 %>% 
  separate(col = address, into = c("1", "two", "three", "four", "5"), sep = ",") %>% 
  select(-c("1", "5")) %>% 
  mutate(two = parse_number(two)
         , three = parse_number(three)
         , four = parse_number(four))
```
