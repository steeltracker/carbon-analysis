...
yearly_scope2 <- plant_emissions_long %>% 
  ...
  summarize(yearly_scope2_co2e = sum(scope2_co2e_tonnes_per_week)) %>% 
  ungroup()

zip_code_tidy <- yearly_scope2 %>% 
  separate(col = address, into = c("zip_code_part1", "zip_code_part2", "zip_code_part3", "zip_code_part4", "zip_code_part5"), sep = ",") %>% 
...
unique_scope2_zip_code <- zip_code_tidy %>% 
  group_by(zip_code) %>% 
  summarise(count = n()) %>% 
  filter(count < 2) %>% 
  na.omit() %>%
...
scope1 <- scope1_readin %>% 
  select(c(zip_code, ghg_quantity_metric_tons_co2e))

unique_scope1_zip_code <- scope1 %>% 
  group_by(zip_code, ghg_quantity_metric_tons_co2e) %>% 
  summarise(count = n()) %>% 
  filter(count < 2) %>% 
  na.omit() %>%
  ungroup() %>% 
...
unique_scope1_2_zip <- unique_scope1_zip_code %>% 
  inner_join(unique_scope2_zip_code, by = "zip_code")

zip_code_scope1_2 <- unique_scope1_2_zip %>% 
  full_join(zip_code_tidy, by = "zip_code")
```
...
# %>% 
#   mutate(plant_id = case_when(plant_name == "" ~ 1003268
#                               , plant_name == "" ~ 1003268
#                               , plant_name == "" ~ 1003268
#                               , plant_name == "" ~ 1003268
#                               , plant_name == "" ~ 1003268
