...
plant_capacity1 <- plant_capacity
plant_capacity1 <- plant_capacity1 %>% 
  mutate(subregion = vector)
```

### Merging data

```{r}
plant_emissions_by_subregion <- left_join(plant_capacity1, subregion_emissions, by = "subregion")
```
...
```{r}
regional_plant_emissions_by_state <- left_join(plant_emissions_by_subregion, AISI_regions, by = "state") %>% 
  select(-c(steel_plant_in_gspt
            , state_abbreviation))

regional_plant_emissions <- regional_plant_emissions_by_state %>% 
...
```{r}
## co2e_per_mwh is from egrid and is pounds of CO2e per MWH of electricty produced
...
## emissions_intensity is pounds of CO2e per tonne of steel
## eap_capacity is in thousand tons per year and we are standardizing to tons per week
regional_plant_emissions <- regional_plant_emissions %>% 
...
plant_emissions <- left_join(regional_plant_emissions, AISI_longer, by = "region") %>% 
  mutate(weekly_co2e = weekly_full_capacity_intensity * utilization)
```
