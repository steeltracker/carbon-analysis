...
```{r}
low_high_emissions_intensity_companies <- rbind(top_emissions_intense_companies, least_emissions_intense_companies) %>% 
  mutate(owner = fct_reorder(owner, avg_emissions_intensity_co2e_tonne_per_tonne))

my_palette <- colorRampPalette(c("#FFFF99", "#FF8C00"))(10)

emissions_intensity_companies_plot <- ggplot(low_high_emissions_intensity_companies, aes(x = avg_emissions_intensity_co2e_tonne_per_tonne
                                                                                    , y = owner
                                                          #                          , fill = my_palette
    )
    ) +
  geom_col(aes(fill = my_palette)) +
  theme_minimal() +
  theme(legend.position = "none")

ggplotly(emissions_intensity_companies_plot)
```
