...
simpley_shapes <- st_as_sf(simple_shapes)

left_join(planty_points, simple_shapes, join = st_within)

st_within(planty_points, simple_shapes)

regional_plant_emissions1 <- regional_plant_emissions
for (i in seq_along(test)) {
   regional_plant_emissions1[ , (ncol(regional_plant_emissions1)+1)[i]] <- simple_shapes$zip_subregi[test[[i]]]
}
```
